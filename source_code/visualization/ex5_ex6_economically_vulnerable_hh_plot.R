...
tract_ev_prev$group_ev1[which(tract_ev_prev$prev1>=breaks_ev1[5] & tract_ev_prev$prev1<breaks_ev1[6])]="[0.196, 0.268)"
tract_ev_prev$group_ev1[which(tract_ev_prev$prev1>=breaks_ev1[6] & tract_ev_prev$prev1<breaks_ev1[7])]="[0.268, 0.378)"
tract_ev_prev$group_ev1[which(tract_ev_prev$prev1>=breaks_ev1[7])]="[0.378, 0.488]"
...
ex6_fairfax_ev_prev_map <- ggplot() + 
  ...
  scale_fill_manual(values=evpalette_orange, 
                    ...
                    guide=guide_legend(
                      ...
                      keywidth=unit(14/length(labels), units="mm"),
                      title.position="top",
                      title.hjust=0.5,
                      label.hjust=1,
                      nrow=1,
...
ggplot(data=ev_plot, aes(x=HLB_prev, y=pov_prev)) +
  geom_segment(aes(x=0, xend=0.71, y=0, yend=0.7), color="black", lty=2) +
  geom_segment(aes(x=0, xend=0.71, y=unname(prev1.lm$coefficients[1]), yend=unname(prev1.lm$coefficients[2])*0.7), color=cbPalette[2]) +
  geom_segment(aes(x=0, xend=0.71, y=unname(prev2.lm$coefficients[1]), yend=unname(prev2.lm$coefficients[2])*0.7), color=cbPalette[3]) +
  ...
  scale_y_continuous(breaks=seq(0, 0.75, by=0.10),
                     labels=seq(0, 0.75, by=0.10),
                     limits=c(-0.07, 0.75)) +
