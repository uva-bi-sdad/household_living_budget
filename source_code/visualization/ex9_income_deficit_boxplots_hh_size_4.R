...
#subtract the poverty threshold from HLB = income deficit
 household_4$dif <- 29782-household_4$HLB_yr; summary(household_4$dif)
...
ggplot(household_4, aes(x=hh_comb, y=dif, fill=puma)) +
 geom_boxplot(aes(x=hh_comb, y=dif), outlier.shape=NA, 
  ...
  scale_y_continuous(limits=c(-100000, -20000),
                     breaks=seq(from=-100000, to=-20000, by=10000)) + 
  theme_minimal() + 
