...
library(dplyr)
 ...
 ipf <- read.csv("https://github.com/uva-bi-sdad/household_living_budget/raw/main/documents/products/derived_variables/fairfax_va_acs_ipf_estimates_2021.csv")
puma <- read.csv("https://github.com/uva-bi-sdad/household_living_budget/raw/main/documents/products/derived_variables/fairfax_va_acs_pums_derived_variables_2021.csv")
puma <- puma[, -1]; puma$hh_comb <- as.factor(puma$hh_comb)
...
#   2844  2676  4094  1974  2659  2145  3511  2522  1998   
...
############################################################################
#synthetic data for 59301 (contains 2844 observations)
...
#check if the puma has a household in each cell and record the missing cells
   missing_cell <- which(apply(ipf_wide_59301[3:no_tracts],1,sum)==0)
          cells <- c(1:45)[-c(missing_cell)]
...
for(j in 3:no_tracts){
  for(i in 1:length(cells)){
        I <- cells[i]; print(I)
        J <- unname(unlist(ipf_wide_59301[I,j])); print(J)
    temp1 <- na.omit(puma59301[puma59301$index==I, ])  
     prob <- temp1$hh_wt/sum(temp1$hh_wt)
    temp2 <- temp1[sample(1:nrow(temp1), size=J, replace=TRUE, prob=prob), ];  
    temp2 <- cbind(tract=rep(names(ipf_wide_59301)[j], J), temp2)
...
############################################################################
#synthetic data for 59302 (contains 2676 observations)
...
#check if the puma has a household in each cell and record the missing cells
missing_cell <- which(apply(ipf_wide_59308[3:no_tracts],1,sum)==0)
       cells <- c(1:45)[-c(missing_cell)]
...
for(j in 3:no_tracts){
  for(i in 1:length(cells)){
        I <- cells[i]; print(I)
        J <- unname(unlist(ipf_wide_59302[I,j])); print(J)
    temp1 <- na.omit(puma59302[puma59302$index==I, ])  
     prob <- temp1$hh_wt/sum(temp1$hh_wt)
    temp2 <- temp1[sample(1:nrow(temp1), size=J, replace=TRUE, prob=prob), ];  
    temp2 <- cbind(tract=rep(names(ipf_wide_59302)[j], J), temp2)
...
############################################################################
#synthetic data for 59303 (contains 4094 observations)
...
#check if the puma has a household in each cell and record the missing cells
missing_cell <- which(apply(ipf_wide_59303[3:no_tracts],1,sum)==0)
       cells <- c(1:45)[-c(missing_cell)]
...
for(j in 3:no_tracts){
  for(i in 1:length(cells)){
        I <- cells[i]; print(I)
        J <- unname(unlist(ipf_wide_59303[I,j])); print(J)
    temp1 <- na.omit(puma59303[puma59303$index==I, ])  
     prob <- temp1$hh_wt/sum(temp1$hh_wt)
    temp2 <- temp1[sample(1:nrow(temp1), size=J, replace=TRUE, prob=prob), ];  
    temp2 <- cbind(tract=rep(names(ipf_wide_59303)[j], J), temp2)
...
############################################################################
#synthetic data for 59304 (contains 1974 observations)
...
#check if the puma has a household in each cell and record the missing cells
missing_cell <- which(apply(ipf_wide_59304[3:no_tracts],1,sum)==0)
       cells <- c(1:45)[-c(missing_cell)]
...
for(j in 3:no_tracts){
  for(i in 1:length(cells)){
           I <- cells[i]; print(I)
           J <- unname(unlist(ipf_wide_59304[I,j])); print(J)
       temp1 <- na.omit(puma59304[puma59304$index==I, ])  
        prob <- temp1$hh_wt/sum(temp1$hh_wt)
       temp2 <- temp1[sample(1:nrow(temp1), size=J, replace=TRUE, prob=prob), ];  
       temp2 <- cbind(tract=rep(names(ipf_wide_59304)[j], J), temp2)
...
############################################################################
#synthetic data for 59305 (contains 2659 observations)
...
#check if the puma has a household in each cell and record the missing cells
missing_cell <- which(apply(ipf_wide_59305[3:no_tracts],1,sum)==0)
       cells <- c(1:45)[-c(missing_cell)]
...
for(j in 3:no_tracts){
  for(i in 1:length(cells)){
        I <- cells[i]; print(I)
        J <- unname(unlist(ipf_wide_59305[I,j])); print(J)
    temp1 <- na.omit(puma59305[puma59305$index==I, ])  
     prob <- temp1$hh_wt/sum(temp1$hh_wt)
    temp2 <- temp1[sample(1:nrow(temp1), size=J, replace=TRUE, prob=prob), ];  
    temp2 <- cbind(tract=rep(names(ipf_wide_59305)[j], J), temp2)
...
############################################################################
#synthetic data for 59306 (contains 2145 observations)
...
#check if the puma has a household in each cell and record the missing cells
missing_cell <- which(apply(ipf_wide_59306[3:no_tracts],1,sum)==0)
       cells <- c(1:45)[-c(missing_cell)]
...
for(j in 3:no_tracts){
  for(i in 1:length(cells)){
        I <- cells[i]; print(I)
        J <- unname(unlist(ipf_wide_59306[I,j])); print(J)
    temp1 <- na.omit(puma59306[puma59306$index==I, ])  
     prob <- temp1$hh_wt/sum(temp1$hh_wt)
    temp2 <- temp1[sample(1:nrow(temp1), size=J, replace=TRUE, prob=prob), ];  
    temp2 <- cbind(tract=rep(names(ipf_wide_59306)[j], J), temp2)
...
############################################################################
#synthetic data for 59307 (contains 3511 observations)
...
#check if the puma has a household in each cell and record the missing cells
missing_cell <- which(apply(ipf_wide_59307[3:no_tracts],1,sum)==0)
       cells <- c(1:45)[-c(missing_cell)] 
...
j_stop <- no_tracts
   for(j in 3:no_tracts){
     for(i in 1:length(cells)){
           I <- cells[i]; print(I)
           J <- unname(unlist(ipf_wide_59307[I,j])); print(J)
       temp1 <- na.omit(puma59307[puma59307$index==I, ])  
        prob <- temp1$hh_wt/sum(temp1$hh_wt)
       temp2 <- temp1[sample(1:nrow(temp1), size=J, replace=TRUE, prob=prob), ];  
       temp2 <- cbind(tract=rep(names(ipf_wide_59307)[j], J), temp2)
...
############################################################################
#synthetic data for 59308 (contains 2522 observations)
...
#get the number of tracts within puma 59308   
   no_tracts <- length(unique(ipf_long_59308$tract))+2
#check if the puma has a household in each cell and record the missing cells
 missing_cell <- which(apply(ipf_wide_59308[3:no_tracts],1,sum)==0)
        cells <- c(1:45)[-c(missing_cell)]
...
j_stop <- no_tracts
   for(j in 3:no_tracts){
     for(i in 1:length(cells)){
           I <- cells[i]; print(I)
           J <- unname(unlist(ipf_wide_59308[I,j])); print(J)
       temp1 <- na.omit(puma59308[puma59308$index==I, ])  
        prob <- temp1$hh_wt/sum(temp1$hh_wt)
       temp2 <- temp1[sample(1:nrow(temp1), size=J, replace=TRUE, prob=prob), ];  
       temp2 <- cbind(tract=rep(names(ipf_wide_59308)[j], J), temp2)
...
############################################################################
#synthetic data for 59309 (contains 1998 observations)
...
#check if the puma has a household in each cell and record the missing cells
missing_cell <- which(apply(ipf_wide_59309[3:no_tracts],1,sum)==0)
       cells <- c(1:45)[-c(missing_cell)]
...
for(j in 3:no_tracts){
  for(i in 1:length(cells)){
        I <- cells[i]; print(I)
        J <- unname(unlist(ipf_wide_59309[I,j])); print(J)
    temp1 <- na.omit(puma59309[puma59309$index==I, ])  
     prob <- temp1$hh_wt/sum(temp1$hh_wt)
    temp2 <- temp1[sample(1:nrow(temp1), size=J, replace=TRUE, prob=prob), ];  
    temp2 <- cbind(tract=rep(names(ipf_wide_59309)[j], J), temp2)
...
fx.syn$serialno <- as.factor(fx.syn$serialno); summary(fx.syn$serialno)
 fx.syn$hh_comb <- as.factor(fx.syn$hh_comb); summary(fx.syn$hh_comb)
